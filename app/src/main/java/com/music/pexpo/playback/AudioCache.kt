 ...
 * original `pexpo://watch?v=<id>` request, and a cache hit never resolves a
 * URL at all.
 */
@UnstableApi
object AudioCache {
     ...
     * [StreamResolver][com.music.pexpo.data.innertube.StreamResolver]'s
     ...
     * `pexpo://watch?v=…` URI it builds for itself carries an id and nothing
     * else. It rides along from the queue instead — see
     * [PlaybackService.prefetchAround][com.music.pexpo.playback.PlaybackService].
     */
     ...
     * [PlaybackService][com.music.pexpo.playback.PlaybackService]'s audition.
    ...
    private suspend fun fetch(videoId: String, position: Long, length: Long) =
        fetch(videoId, Uri.parse("pexpo://watch?v=$videoId"), position, length)
