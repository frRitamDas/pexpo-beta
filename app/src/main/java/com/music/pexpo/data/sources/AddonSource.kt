...
import com.music.pexpo.data.sources.addon.AddonNotFound
import com.music.pexpo.data.sources.addon.AddonStream
import com.music.pexpo.data.sources.addon.AddonTrack
import com.music.pexpo.playback.StreamContainer
...
class AddonSource(
     ...
     * [QualityUpgrade][com.music.pexpo.playback.QualityUpgrade] checks a
     ...
     * [TrackMatcher][com.music.pexpo.data.sources.TrackMatcher] decides which
    ...
    private fun openable(url: String, answer: AddonStream, trackId: String, tier: String): SourceStream? {
        ...
        if (answer.isEncrypted) {
            TrackLog.w(
                TAG,
                "${config.displayName}: $trackId came back encrypted, which Pexpo never asked for — passing",
            )
            return null
        }
