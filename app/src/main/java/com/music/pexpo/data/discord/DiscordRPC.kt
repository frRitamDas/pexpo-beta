package com.music.pexpo.data.discord
 ...
 *
 * ```
 *   Listening to Pexpo          <- activityName, or the app's own name
 *   ┌────┐  Song title             <- details
 *   │art │  Artist                 <- state
 *   └────┘  ▁▁▁▁▁▁ 1:04 / 3:47     <- from the timestamps
 *   [ Listen on YouTube Music ]    <- button 1
 *   [ Visit Pexpo           ]   <- button 2
 * ```
 */
class DiscordRPC(
    val context: Context,
     ...
     * a side-by-side dev install should still look like pexpo to everyone
     * else on Discord.
     */
    ...
    companion object {
        ...
        private const val FALLBACK_ART_URL =
            "https://raw.githubusercontent.com/kushagrasinghx/Pexpo/main/app/src/main/ic_launcher-playstore.png"
