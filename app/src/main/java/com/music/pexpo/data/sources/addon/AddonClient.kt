package com.music.pexpo.data.sources.addon

import com.music.pexpo.data.Http
import com.music.pexpo.data.TrackLog
import com.music.pexpo.data.sources.module.SharedCalls
 ...
 * this side. Compare [ModuleManager][com.music.pexpo.data.sources.module.ModuleManager],
...
class AddonClient(rawBaseUrl: String) {
    ...
    suspend fun manifest(): Result<AddonManifest> = manifests.get(
        ...
        fetch<AddonManifest>(manifestUrl(baseUrl)).mapCatching { manifest ->
            ...
            if (!manifest.isPlayable) {
                ...
                throw AddonException("This addon declares $declared — Pexpo needs search")
            }
            manifest
        ...
        while (true) {
            ...
            val wait = client.newCall(request).execute().use { response ->
                when {
                    ...
                    attempt >= MAX_RETRIES -> throw AddonUnavailable("This addon is rate limiting Pexpo")
    ...
    companion object {
        ...
        /** The tiers [AddonSource][com.music.pexpo.data.sources.AddonSource] asks in. */
 ...
 * [SourceHealth.Rejected][com.music.pexpo.data.sources.SourceHealth.Rejected],
 ...
 * [SourceHealth][com.music.pexpo.data.sources.SourceHealth] draws.
 */
